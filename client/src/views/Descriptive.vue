        ...
        <div class="grid">
            ...
            <div class="cell -4of12">
                <button class="btn btn-default"
                    @click="setDescriptive">Calculate</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async setMean() {
            const { body: mean } = await this.$http.post(
                "/api/descriptive/mean"
            );
            this.mean = mean.toFixed(2);
        },

        async setStandardDeviation() {
            const { body: standardDeviation } = await this.$http.post(
                "/api/descriptive/standard-deviation"
            );
